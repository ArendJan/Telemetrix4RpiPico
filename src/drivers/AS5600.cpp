...
AS5600::AS5600(uint8_t busnum) { _busnum = busnum; }
...
bool AS5600::isConnected() { return check_addr(this->_busnum, _address); }

uint8_t AS5600::getAddress() { return _address; }
...
uint8_t AS5600::getPowerMode() { return readReg(AS5600_CONF + 1) & 0x03; }
...
uint8_t AS5600::getSlowFilter() { return readReg(AS5600_CONF) & 0x03; }
...
uint8_t AS5600::getFastFilter() { return (readReg(AS5600_CONF) >> 2) & 0x07; }
...
uint8_t AS5600::getWatchDog() { return (readReg(AS5600_CONF) >> 5) & 0x01; }
...
float AS5600::getOffset() { return _offset * AS5600_RAW_TO_DEGREES; }
...
bool AS5600::magnetTooWeak() { return (readStatus() & AS5600_MAGNET_LOW) > 1; }
...
uint8_t AS5600::readReg(uint8_t reg) {
  _error = AS5600_OK;
  auto data = std::vector<uint8_t>{reg};
  auto ok = read_i2c(_busnum, _address, data, 1, data);
...
uint16_t AS5600::readReg2(uint8_t reg) {
  _error = AS5600_OK;
  auto data = std::vector<uint8_t>{reg};
  auto out = std::vector<uint8_t>(2);
  auto ok = read_i2c(_busnum, _address, data, 2, out);
  //   _wire->beginTransmission(_address);
  //   _wire->write(reg);
  //   if (_wire->endTransmission() != 0)
  //   {
...
uint8_t AS5600::writeReg(uint8_t reg, uint8_t value) {
  _error = AS5600_OK;
  auto data = std::vector<uint8_t>{reg, value};
  auto ok = write_i2c(_busnum, _address, data);
  //   _wire->beginTransmission(_address);
  //   _wire->write(reg);
  //   _wire->write(value);
  if (!ok) {
...
uint8_t AS5600::writeReg2(uint8_t reg, uint16_t value) {
  _error = AS5600_OK;
  auto data =
      std::vector<uint8_t>{reg, (uint8_t)(value >> 8), (uint8_t)(value & 0xFF)};
  auto ok = write_i2c(_busnum, _address, data);
...
//  AS5600L
//
AS5600L::AS5600L(uint8_t address, uint8_t busnum) : AS5600(busnum) {
  _address = address;
...
uint8_t AS5600L::getI2CUPDT() { return (readReg(AS5600_I2CUPDT) >> 1); }

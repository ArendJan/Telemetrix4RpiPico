...
public:
  ...
  TCA9548(uint8_t deviceAddress, uint8_t busnum);
  ...
  bool isConnected(uint8_t address,
                   uint8_t channel); //  find address on selected channel
...
protected:
  uint8_t _address;
  uint8_t _busnum;   //  I2C bus number
...
public:
  PCA9548(uint8_t deviceAddress, uint8_t busnum);
};
